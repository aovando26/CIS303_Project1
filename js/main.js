...
// assign varaibls to value 0 
let annualUseKw = 0, dailyUseKw = 0, i = 0, x = 0;

// reference the element by ID, then by tag name 
let months = document.getElementById(elem).getElementsByTagName('input');

// use a for loop to iterate over the value of months, gathering its total 
	for(i = 0; i <months.length; i++) {
		x = Number(months[i].value);
		annualUseKw += x; 
	}

// divide the annual usage with the days of the year 	
...
function calculateSolar() {
	...
	// pass an argument value to the function addMonths storing in the variable 
